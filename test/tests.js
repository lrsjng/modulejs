...
describe('modulejs', function () {
    ...
    beforeEach(function () {
        modjs = root.modulejs.create();
    });

    it('is published to the root object', function () {
        assert.ok(_.has(root, 'modulejs'));
    });

    it('is the only published object', typeof window === 'object' ? undefined : function () {
        assert.deepEqual(_.keys(root), ['modulejs']);
    });
